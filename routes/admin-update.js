...
router.post('/', async(req, res) => {
  try {
    ...
    const pollCreator = details.poll_creator_id;
    ...
    if (newEmails.length > 0) {
      ...
      await addAuthorizedToVote(authorizedIds, pollId);
    }
    ...
    if (deletedEmails.length > 0) {
      ...
      const newEmailsAdded = await addNewEmails(emailsToAdd);
      newEmailsAdded.forEach(obj => authorizedIds.push(obj.id));
      await removeAuthorizedEmail(authorizedIds, pollId);
      await removeUnauthorizedVotes(authorizedIds, pollId);
    }
    ...
    await updateOptions(existingOptions);
